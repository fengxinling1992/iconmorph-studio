...
export function gradientPresetUpdates(preset: GradientPreset, style: StyleId): Partial<RenderParams> {
  ...
  if (style === "nebula") return { nebulaPrimary: preset.start, nebulaSecondary: preset.end, nebulaAngle: 135 };
...
export function makeMasterGoCompatibleSvg(svg: string) {
  ...
  const pendingAttributes = new Map<Element, Map<string, string>>();
  for (const rule of rules) {
    for (const element of elements) {
      if (!rule.selectors.some((selector) => { try { return element.matches(selector); } catch { return false; } })) continue;
      const attributes = pendingAttributes.get(element) ?? new Map<string, string>();
      rule.declarations.forEach(([property, value]) => attributes.set(property, value));
      pendingAttributes.set(element, attributes);
    }
  }
  pendingAttributes.forEach((attributes, element) => attributes.forEach((value, property) => element.setAttribute(property, value)));
...
export default function Home() {
  ...
  const resetActiveStyleParams = () => {
    const resetKeys: Record<StyleId, Array<keyof RenderParams>> = {
      ...
      nebula: ["nebulaPrimary", "nebulaSecondary", "nebulaAngle", "nebulaShape", "nebulaGlassOpacity", "nebulaBlur", "nebulaShadow", "nebulaHighlight"],
    };
    ...
    <div className="app-shell">
      ...
      <main className="workspace">
        ...
        <aside className="control-rail">
          ...
          {selectedStyle === "nebula" && <><div className="parameter-group"><div className="group-title"><Sparkles size={15}/><span>星云纯色背景</span></div><div className="scene-kit-options scene-kit-options-three nebula-shape-options"><button type="button" onClick={() => updateParam("nebulaShape", "square")} className={params.nebulaShape === "square" ? "scene-kit-active" : ""}>正方形</button><button type="button" onClick={() => updateParam("nebulaShape", "circle")} className={params.nebulaShape === "circle" ? "scene-kit-active" : ""}>圆形</button><button type="button" onClick={() => updateParam("nebulaShape", "triangle")} className={params.nebulaShape === "triangle" ? "scene-kit-active" : ""}>三角形</button></div><GradientPresetPicker valueStart={params.nebulaPrimary} valueEnd={params.nebulaSecondary} onChange={applyGradientPreset} /><div className="color-row"><ColorField label="背景色" color={params.nebulaPrimary} onChange={(value) => updateParam("nebulaPrimary", value)} /><ColorField label="玻璃色" color={params.nebulaSecondary} onChange={(value) => updateParam("nebulaSecondary", value)} /></div><SliderField label="渐变角度" value={params.nebulaAngle} min={0} max={360} suffix="°" onChange={(value) => updateParam("nebulaAngle", value)} /></div><div className="parameter-group"><div className="group-title"><WandSparkles size={15}/><span>毛玻璃层</span></div><SliderField label="玻璃不透明度" value={params.nebulaGlassOpacity} min={35} max={100} suffix="%" onChange={(value) => updateParam("nebulaGlassOpacity", value)} /><SliderField label="背景模糊" value={params.nebulaBlur} min={0} max={24} suffix=" px" onChange={(value) => updateParam("nebulaBlur", value)} /><SliderField label="阴影模糊" value={params.nebulaShadow} min={0} max={16} suffix=" px" onChange={(value) => updateParam("nebulaShadow", value)} /><SliderField label="边缘高光" value={params.nebulaHighlight} min={0} max={100} suffix="%" onChange={(value) => updateParam("nebulaHighlight", value)} /></div></>}
          {selectedStyle === "scene" && <>
